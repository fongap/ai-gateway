...
- `wrangler.jsonc.name` is the default Worker name, not the GitHub repository name. The npm `postbuild` lifecycle synchronizes it to the connected Worker only inside Cloudflare's temporary build workspace.
