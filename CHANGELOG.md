...
>
> 版本序列说明：`v1.2.7` 为 legacy/v1.2.7 分支维护版本；当前 mainline 从 v1.2.6 基线继续演进并收口为 **v1.3.0**。
...
- **Deploy 工作流由 CI 的 `workflow_run` 完成事件触发,完整验证每次 push 只执行一次。** CI 的 `validate-merge` + `validate-deploy` 两个 job 全部成功 = Production Gate——Worker Deploy 绝不在此之前执行;Merge Gate(`validate-merge`)≠ Production Gate。gate 决策收敛至 `scripts/deploy-gate-decision.mjs`(可单测),规则:① 仅 **push 触发**的 CI 成功允许自动部署——nightly(schedule)与手动触发的 CI 只测试、永不部署(gate 显式判断 `workflow_run.event == push`,不做 commit message/时间/分支猜测);② fork head repo 永不部署;③ CI conclusion ≠ success 永不部署;④ 仅改动 `**.md`/`docs/**` 的触发 commit 沿用原 `paths-ignore` 跳过策略。**手动 Deploy 不再是隐式 bypass**:新增 `manual-validate` job 在 deploy 工作流内先跑完整验证(`validate:deploy` + typecheck + strict typecheck + bundle dry-run),deploy job 依赖其成功;自动路径跳过该 job、复用已完成的 CI。**依赖跳过语义**:`deploy` 的 `if` 必须携带 `always()`——被 needs 的 job 被 skip 时,GitHub 会整体跳过依赖 job,仅引用 `needs.X.result` 不会生效(此规则缺失曾导致自动部署路径被静默跳过);安全性由显式守卫承担:gate 必须输出 `deploy=true`,手动路径 `manual-validate` 必须真实 `success`(cancelled/failed 仍阻断)。本轮不引入 break-glass——如需紧急部署,以后以显式 `break_glass` 输入或独立 workflow 设计。D1 migration 步骤移至 Worker Deploy **之前**:`migration 失败 → deploy 不发生`,不再出现"新代码 + 旧 Schema"线上状态;`TOKEN_STATS_D1_ID` 未配置时 migration 步骤跳过、不阻断部署。回滚语义不变(仅已部署 + 后续步骤失败时 `wrangler rollback`,变量/Secrets/迁移不回滚)。
- `docs/operations/deployment.md` 部署顺序与触发事件规则同步更新;README / README_EN 标注 Production Gate 与迁移顺序。
- **Deployment Workflow Contract Test 扩展至 ×11**(`scripts/deployment-workflow-contract-test.mjs`):C01–C07 保留(migration 顺序、Production Gate、回滚语义等,gate 断言改为行为化驱动决策函数),新增 C08 push/main/success → 允许部署、C09 schedule/success → 阻断、C10 手动 CI/success → 阻断、C11 手动 Deploy 必须先通过 `manual-validate` 完整验证(内联 bypass 脚本禁止回归)。
- **Release / Tag 生命周期治理**:`docs/governance/release-policy.md` 固化 Squash Merge 下的 tag 纪律——正式 release 顺序为 PR Merge(squash)→ main 完整 CI → Production Deploy 成功 → 确认 final main SHA → 打 tag → GitHub Release;禁止在 PR merge 前打 tag、禁止在 PR 分支上打/重建 tag;未发布 GitHub Release 的提前 tag 删除重建至 final main SHA,已发布 Release 的 tag 视为外部契约不可移动;本轮 correctness fix 保持 1.2.6,下一功能版本号单独决策。
...
### Changed — Calendar Heatmap 正确性(P1)

- **月份标签 Renderer 修复(右边界月份丢失)**:删除 renderer 中 `weekIndex > weeksCount - 3 → break` 的丢标签逻辑——月份标签是日期语义,`9月` 锚定在包含 2026-09-01 的最后一列(weekIndex 51)时必须出现在输出中,排版溢出由 CSS 解决而非删标签。两种模式统一为单循环输出(calendar-year 恒 12 个标签,1月/12月不得因边缘丢失)。
- **`.months` flex → 与 heatmap 同列 CSS Grid**:原先 `display:flex; justify-content:space-between` 使标签的 `grid-column` 完全失效("数据正确、页面显示错误")。现 `.heatmap` 与 `.months` 共用 `grid-template-columns:repeat(var(--week-count,52),10px)`(由消费方 inline 注入 `--week-count`),标签 `grid-column` 即真实定位;`.months span` 增加 `white-space:nowrap` 防 10px 轨道内换行。
- **Cell 位置事实源 = HeatmapDay**:每个 cell 携带 `data-week` / `data-weekday` 并 inline 显式 `grid-column` / `grid-row` 定位,布局不再依赖 DOM 顺序 / `grid-auto-flow`;52/53/54 列复用同一 renderer。CSS 轻微区分三种状态:真实 0 activity(level-0 灰)、future(`data-future="1"`,空白+描边)、range 外占位(`data-inrange="0"`,近透明);配色体系不变。
- **Dashboard 首页标题统一**:`Token 活动 · 52 周` → `Token 活动 · 近 52 周`(明确 rolling-52-weeks 语义)。calendar-year 模式保持 Builder/Renderer/测试完整,本轮不新增年份选择器等 UI。
- **热力图铺满内容宽度(用户反馈)**:week 轨道由固定 `10px` 改为 `1fr`——heatmap 与 months 行共用相同的 `--week-count` 轨道,cell 拉伸填满轨道(高度保持 10px),52 列铺满整行不再留白;月份标签与列对齐关系不变。C15 契约同步(cell 不得再固定 `width:10px`)。
- **模型使用面板按官方逻辑 ID 显示(用户反馈)**:D1 统计聚合键是 canonical 小写,但展示层此前直接渲染小写统计键,与「模型状态」区的官方 ID(如 `Code-Max`)大小写不一致。现在 pages.js 以 node mappings(与模型状态同源)构建 canonical key → 官方 ID 映射传入 usage 面板,donut/bars/tooltip 全部显示 `Code-Max` 等官方名称;未在任何 node 声明的统计键回退显示原始键。统计/匹配逻辑不变,仅展示层重命名。
- **测试 helper 时区契约修正**:测试辅助 `dateAtIso` 由 `${iso}T00:00:00Z`(实际是 UTC+8 当日 08:00,边界测试"碰巧通过")改为真正的 UTC+8 午夜 `${iso}T00:00:00+08:00`(= 前一日 16:00Z),UTC+8 日期边界真正被覆盖。
- **Calendar Heatmap Contract ×15**(`scripts/calendar-heatmap-contract-test.mjs`,已纳入 test:unit):C01 rolling 恒 52 列;C02 Monday-first;C03 业务 key = YYYY-MM-DD;C04 future ≠ 0;C05 月份标签 = monthStart weekIndex;C06 右边界月份不丢;C07 calendar-year range = Jan1–Dec31;C08 列数动态且精确(非闰年恒 53,闰年且 Jan1=周日 → 54,固定案例 2012/2040=54、2015/2016/2019/2026/2027/2028=53);C09 年度占位 inRange=false;C10 当前年未来 = future/null;C11 历史年无 future;C12 闰日(2028-02-29,366 天);C13 UTC+8 午夜边界(15:59:59Z / 16:00:00Z,日期 ±1、星期/月份不偏移);C14 renderer 与 builder 同列;C15 months CSS 不得回退 flex/space-between。calendar-heatmap-test.mjs 同步废除 "53 或 54 都算对" 宽松断言。
...
- **调度器升级为 protocol + surface + model 三重过滤**(`supportsRequest`)。`/v1/responses` 只路由到 `surfaces` 含 `responses` 的 openai 节点,`/v1/messages` 优先进入 anthropic 节点。调度器自身从不执行跨协议选择——每次调度 pass 严格限定于一个 `(protocol, surface, model)` descriptor;跨协议 fallback 在调度器之外由 request 层编排,仅在 `PROTOCOL_FALLBACKS` 显式声明时以新 descriptor 重新进入调度器。同协议内的轮换/hedge/tier fallback 完整保留。
...
---
### v1.2.7 Legacy Release & v1.3.0 Mainline Convergence

GitHub 上已存在 `v1.2.7` 标签（legacy/v1.2.7 分支维护版本），它是一个独立的维护分支发布。

当前 mainline 从 v1.2.6 基线继续演进并收口为 **v1.3.0**。下方汇总的变更已全部合并至 mainline，不再使用 "1.2.7" 版本号。

#### Changed — Model Governance & Consistency Convergence (merged to v1.3.0)

#### Fixed — Observability Consistency Convergence (merged to v1.3.0)

#### Changed — Protocol Docs & Version Convergence (merged to v1.3.0)

- **Protocol architecture unified**: OpenAI / Anthropic dual-native, Native First; only Anthropic Messages may fallback to OpenAI Chat when `PROTOCOL_FALLBACKS` explicitly configured; no implicit cross-protocol fallback; Hedge never crosses protocol / surface.
- **Version narrative**: v1.2.7 is the legacy/v1.2.7 branch release; current mainline evolves from v1.2.6 baseline converging to v1.3.0.

#### Removed

- Removed stale converter code and unused compatibility layers.
- Dashboard layout frozen (width/px/spacing/font) — focus on governance correctness.
