...
Fair-share failover budgets, re-tuned timeout defaults, a reworked public homepage (usage KPIs + 52-week activity heatmap), and durable cross-isolate token usage. No new protocols or providers.

### Fixed

- **Security scan no longer rejects its own gitignored deployment config.** After local configuration generated `wrangler.user.jsonc`, `npm run verify` always failed because the scanner walked every file on disk even though the file was explicitly gitignored. In a Git worktree the scan now covers tracked files plus non-ignored untracked files—the exact commit candidate set. A forcibly tracked sensitive file still fails; source archives without Git retain the conservative full-tree fallback.
- **P1 — Large Responses terminal events no longer look truncated.** Stream tracking previously retained only the last 256 decoded characters *before* looking for `event: response.completed`; because that terminal event contains the full response object, its header was routinely outside the retained suffix. Successful `/v1/responses` streams were therefore logged as `missing_completion_marker`, penalized, cooled, and could eventually open a circuit. Completion/error markers are now scanned in the full current chunk plus the boundary tail before the suffix is trimmed.
- **P1 — Fair timeout shares use live attempt capacity and one absolute attempt deadline.** The initial fix divided header waits by the policy's configured `max_attempts`, which shortened a one-node pool as if five nodes existed and left most of the failover budget unused. The divisor is now recomputed from live dispatchable nodes constrained by tier and shared caps. Each dispatch receives one fair wall-clock slice: response headers and the first-event guard consume the same deadline, so the two serial phases cannot each spend a fresh share and starve later candidates.
- **Explicit, non-duplicated mid-stream failure events.** After any client-visible bytes, transparent failover remains forbidden, but reader errors, idle timeouts, and clean EOF without a completion marker now append a protocol-shaped error event before closing (Chat, Responses, or Anthropic). A Responses transform that already emitted `response.failed` is recognized as terminal instead of receiving a second `event: error`; injected Responses errors continue the observed `sequence_number`. The replay guard also preserves a hidden upstream reader exception, so metrics report `reader_error` rather than misclassifying it as `missing_completion_marker`.
- **P1 — One slow node can no longer starve every later candidate.** Each attempt's response-header wait was `min(UPSTREAM_HEADERS_TIMEOUT_MS, remaining budget)`, so with the defaults (120s headers / 180s budget) the first node could burn 120s on a timeout and leave the second node only ~60s — a two-timeout request returned 504 with half its budget spent on a single candidate (`failure_kinds: {"timeout":2}`). The header wait is now the remaining budget divided evenly across the attempts that may still be needed (floored at 20s, still capped by `UPSTREAM_HEADERS_TIMEOUT_MS` and by the remaining budget; the last remaining attempt keeps the old full-remaining behavior exactly). Operators who raise `FAILOVER_BUDGET_MS` no longer just move the failure later — every candidate gets real time.
...
- **Public homepage accessibility and mobile polish.** Model availability now includes screen-reader text instead of relying on color/title alone; quick-start tabs expose complete tab/tabpanel state and support Left/Right/Home/End keyboard navigation; the 52-week heatmap becomes a labelled, horizontally scrollable figure on narrow screens instead of collapsing into near-invisible cells; mobile KPI dividers now align with the actual two-column grid. Text/accent colors retain the Flyme-style palette while meeting practical small-text contrast targets, with visible focus rings and forced-colors support. Copy feedback now says `复制失败` when both Clipboard API and the fallback fail instead of falsely claiming success.
- **Timeout defaults re-tuned for slow upstreams and long-reasoning models.** `UPSTREAM_HEADERS_TIMEOUT_MS` 120s → 60s, `FIRST_EVENT_TIMEOUT_MS` 60s → 120s, `STREAM_IDLE_TIMEOUT_MS` 90s → 240s, `FAILOVER_BUDGET_MS` 180s → 240s. Combined with the fair share above, a default five-attempt request reserves about 48s of response-header wait for each candidate instead of letting the first consume 120s and leaving the second only ~60s; a mid-stream pause of up to 4 minutes no longer truncates the output. All four remain per-deployment env overrides (clamps unchanged).
- **Stalling-stream accounting hardened.** Field incidents (an upstream emitting its first chunks then going silent until the idle timeout) showed the old response was too soft: a 2s stream-failure cooldown put the stalling node straight back into rotation, and the 120s idle timeout kept the client waiting two extra minutes on a provably dead stream. `stream_interrupted` failures now cool the node for 60s (matching the rate-limit cooldown). Both stay configurable for deployments with different tolerance profiles.
...
- **Cross-isolate durable token usage on the public homepage.** The homepage's 使用情况 card no longer reads isolate-local memory (which resets on every isolate restart and never aggregates across PoPs). It now reads an **optional, fail-open** Cloudflare D1 hourly aggregate (`TOKEN_STATS_DB`): only UTC-hour buckets are stored (`token_usage_hourly`), updated by a single atomic `INSERT ... ON CONFLICT(hour) DO UPDATE` UPSERT, and queried server-side for cumulative / 24h / 7d totals plus cumulative requests, a today total, and a 52-week Token activity heatmap. Isolate-local `summarizeTokenStats()` stays for `/metrics` and `/health`. D1 is a pure observability side channel: it is written via `ctx.waitUntil()`, never awaited before a response, and a missing/failing binding degrades the panel to "统计暂不可用" with em dashes (never a fake `0`) without affecting any AI request, fallback, node health, circuit/cooldown, scheduler, concurrency count or stream completion. No per-request rows, no provider/node/tier/key/user/IP dimensions; tokens are upstream-reported usage only, never estimated.
- **Streaming usage capture.** OpenAI-compatible streaming requests now ask the upstream to report usage via `stream_options: { include_usage: true }` (preserving any client `stream_options`; never overwriting a client `include_usage`; non-stream untouched). Gated by the provider profile capability and operator switches (`STREAM_INCLUDE_USAGE`, `STREAM_USAGE_INCLUDE_OFF_PROVIDERS`), so a provider that rejects the field can be opted out without hardcoding a node id. The public homepage shows the durable aggregate; usage normalization and the reported-vs-missing gate are unchanged.
