...
// Top-level `thinking` is a request-control setting. Generic OpenAI-compatible
// Chat providers have no portable equivalent, so the fallback accepts and
// deliberately drops that setting. Anthropic thinking CONTENT blocks remain
// non-convertible because dropping message history would lose semantics.
...
console.log('anthropic thinking fallback test passed');
